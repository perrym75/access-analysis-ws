...
public class DepartmentRepository implements HierarchicalRepository<Department> {
    ...
    public List<Department> findChildren(long id) throws SQLException, IOException {
        ...
        try (
                Connection conn = DriverManager.getConnection(url);
                PreparedStatement st = conn.prepareStatement(
                        TextResourceLoader.loadResource(getQueryResourceName("selectChildDepartments")))
        ) {
            st.setLong(1, id);
            try (ResultSet rs = st.executeQuery()) {
                while (rs.next()) {
    ...
    public List<Department> findRoot() throws SQLException, IOException {
        ...
        try (
                ...
                ResultSet rs = st.executeQuery(
                        TextResourceLoader.loadResource(getQueryResourceName("selectRootDepartments")))
        ) {
            while (rs.next()) {
                entities.add(createEntity(rs));
            }
    ...
    public Department findOne(long id) throws SQLException, IOException {
        try (
                Connection conn = DriverManager.getConnection(url);
                PreparedStatement st = conn.prepareStatement(
                        TextResourceLoader.loadResource(getQueryResourceName("selectDepartment")))
        ) {
            st.setLong(1, id);
            try (ResultSet rs = st.executeQuery()) {
                if (rs.next()) {
