...
public class UnitRepository implements HierarchicalRepository<Unit> {
    ...
    public List<Unit> findRangeOfAll(long from, long count) throws SQLException, IOException {
        ...
        try (
                ...
                PreparedStatement st = conn.prepareStatement(
                        CachedResourceReader.readString(getQueryResourceName("selectRangeOfUnits")))
        ) {
            st.setLong(1, from + 1);
    ...
    public List<Unit> findChildren(long id) throws SQLException, IOException {
        ...
        try (
                Connection conn = DriverManager.getConnection(url);
                PreparedStatement st = conn.prepareStatement(
                        CachedResourceReader.readString(getQueryResourceName("selectChildUnits")))
        ) {
            st.setLong(1, id);
            try (ResultSet rs = st.executeQuery()) {
                while (rs.next()) {
    ...
    public List<Unit> findRoot() throws SQLException, IOException {
        ...
        try (
                ...
                ResultSet rs = st.executeQuery(
                        CachedResourceReader.readString(getQueryResourceName("selectRootUnits")))
        ) {
            while (rs.next()) {
                entities.add(createEntity(rs));
            }
    ...
    public Unit findOne(long id) throws SQLException, IOException {
        try (
                Connection conn = DriverManager.getConnection(url);
                PreparedStatement st = conn.prepareStatement(
                        CachedResourceReader.readString(getQueryResourceName("selectUnit")))
        ) {
            st.setLong(1, id);
            try (ResultSet rs = st.executeQuery()) {
                if (rs.next()) {
    ...
    public List<Unit> findByAgent(long id) throws SQLException, IOException {
        ...
        try (
                Connection conn = DriverManager.getConnection(url);
                PreparedStatement st = conn.prepareStatement(
                        CachedResourceReader.readString(getQueryResourceName("selectUnitsByAgent")))
        ) {
            st.setLong(1, id);
            try (ResultSet rs = st.executeQuery()) {
                while (rs.next()) {
