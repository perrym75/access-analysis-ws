...
IF @model_id = 0
  BEGIN
    ...
    FROM
      ROLE2 role
      INNER JOIN LINK_ROLE2_RESOURCE lrr ON role.ROLE2_ID = lrr.ROLE2_ID
      INNER JOIN [RESOURCE] res ON lrr.RESOURCE_ID = res.RESOURCE_ID
      INNER JOIN RESOURCE_TYPE rt ON res.RESOURCE_TYPE_ID = rt.RESOURCE_TYPE_ID
    WHERE
      role.ROLE2_ID = @role_id
  END
ELSE
  BEGIN
    SELECT
